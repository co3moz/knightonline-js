...
Login server is the first endpoint to reach. When user opens launcher, _tcp_ connection will be establish between server and client. Server port is _15100_ as default. But this port value is changed in later versions to help load balancing. This project will cover all ports that required. New port range is starts with _15100_ ends with _15109_. Launcher will pick random port when starts.

### Game server

Game server is the deal. After user logins, login server advises to connect game server. Game server always works at _15001_ port. If client cant reach the server, user will see _10061_ or similiar error that tells couldn't reach the server. (10061 is tcp error code)
...
- All messages has to be start with _0xAA_ and _0x55_ bytes. These bytes represent 10101010 and 01010101.
- All messages has to be end with _0x55_ and _0xAA_ bytes. These bytes are reversed of start bytes.
