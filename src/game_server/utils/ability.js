...
exports.calculateUserAbilities = socket => {
  let c = socket.character;
  let v = socket.variables;

  let coefficient = coefficientTable[c.klass];
  ...
  defineIfNotExists(v, 'addWeaponDamage', 0);
  defineIfNotExists(v, 'acAmount', 0);
  ...
  if (!v.weaponsDisabled) {
    ...
    if (rightHandItem && rightHandItem.detail) {
      switch (rightHandItem.detail.kind / 10 >>> 0) {
        ...
        case 4:
        case 18: hitCoefficient = coefficient.mace; break;
        case 5:
        case 7:
        case 10:
  ...
  exports.calculateStatBonus(socket);
  ...
  exports.setMaxHp(socket, 0);
  exports.setMaxMp(socket);

  v.resistanceBonus = 0;
  // TODO: resistance bonus calculate
}

var defineIfNotExists = (obj, key, value) => {
...
exports.setItemApply = (setItem, v) => {
  if (!setItem) return;
