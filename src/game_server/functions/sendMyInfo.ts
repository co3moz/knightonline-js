...
export function SendMyInfo(socket: IGameSocket) {
  ...
  for (let i = 0; i < 75; i++) {
    ...
    if (item) {
      items.push(
        ...
        0,
        0,
        0, // TODO: seal serial data
        0,
        0,
        0,
        0 // unix expiration time
      );
    } else {
  ...
  socket.send([
    ...
    ...int(v.itemWeight),
    c.statStr,
    v.statBonus[0] + v.statBuffBonus[0],
    c.statHp,
    v.statBonus[1] + v.statBuffBonus[1],
    c.statDex,
    v.statBonus[2] + v.statBuffBonus[2],
    c.statInt,
    v.statBonus[3] + v.statBuffBonus[3],
    c.statMp,
    v.statBonus[4] + v.statBuffBonus[4],
    ...short(v.totalHit),
    ...short(v.totalAc),
    v.fireR,
    v.coldR,
