...
export const ITEM_REMOVE: IGameEndpoint = async function (socket: IGameSocket, body: Queue, opcode: number) {
  let type = body.byte();
  let pos = body.byte();
  let itemID = body.int();
  ...
  try {
    ...
    if (type == 0 || type == 2) { // item in inventory
    ...
    } else if (type == 1) { // item in equiped area
