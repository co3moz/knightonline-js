...
export const DROP_TAKE: IGameEndpoint = async function (
  ...
  try {
    ...
    if (
      !drop.owners ||
      drop.owners.findIndex((owner) => owner == session) == -1
    )
      throw 1;
    ...
    if (dropItem.item == 900000000) {
      ...
      let newTotalWeight =
        itemWeight + (itemDetail.weight | 0) * dropItem.amount;
      ...
      if (c.items[slot]) {
        c.items[slot].amount = Math.min(
          9999,
          c.items[slot].amount + dropItem.amount
        );
      } else {
