...
export const SHOPPING_MALL: IGameEndpoint = async function (
  socket: IGameSocket,
  body: Queue,
  opcode: number
) {
  ...
  if (subOpcode == 2) {
    ...
    return socket.send([opcode, subOpcode, ...data]);
  }

  // STORE_LETTER
  let letterOperation = body.byte();

  if (letterOperation == 1) {
    // LETTER_UNREAD
    return socket.send([
      opcode,
      subOpcode,
  ...
  } else if (letterOperation == 2 || letterOperation == 3) {
    // LETTER_LIST or HISTORY
    ...
    return socket.send([
      ...
      ...[].concat(
        ...mails.map((mail) => [
          ...
          ...int(
            mail.createdAt.getFullYear() * 10000 +
              (mail.createdAt.getMonth() + 1) * 100 +
              mail.createdAt.getDate()
          ),
          7 - (Date.now() - mail.createdAt.getTime()) / 1000 / 3600 / 24,
        ])
      ),
    ]);
  } else if (letterOperation == 5) {
    // LETTER_READ
    let marker = body.int();
    ...
    if (!mail) {
      return socket.send([opcode, subOpcode, letterOperation, 0]);
    }
    ...
    return socket.send([
      ...
      1,
      ...int(marker),
      ...byte_string(mail.message),
    ]);
  } else if (letterOperation == 6) {
    // SEND
    // TODO: need to do letter send
  } else if (letterOperation == 4) {
    // GET ITEM
    // TODO: need to do letter get item
  } else if (letterOperation == 7) {
    // DELETE
    let count = body.byte();

    if (count > 5) {
      return socket.send([opcode, subOpcode, letterOperation, -3 & 0xff]);
    }
  }
