...
import { SendUsageMessageForGM, GM_COMMANDS_HEADER, GM_COMMANDS } from "../functions/GMController";
import { ChatMessageType, SendPlayerMessageToRegion, SendMessageToPlayerFromPlayer } from "../functions/sendChatMessage";
import { RUserMap } from "../region";

export const CHAT: IGameEndpoint = async function (socket: IGameSocket, body: Queue, opcode: number) {
  let type = body.byte();
  ...
  if ((type == 2 && socket.character.gm && socket.variables.chatTo == GM_COMMANDS_HEADER) ||
    ...
    if (!GM_COMMANDS[command]) {
      return SendUsageMessageForGM(socket, `ERROR: Invalid command "${command}"`);
    }
  ...
  if (type == ChatMessageType.PRIVATE) {
    ...
    SendMessageToPlayerFromPlayer(userRegionContainer.socket, socket, ChatMessageType.PRIVATE, message);
  }
}
