...
export const ITEM_TRADE: IGameEndpoint = async function (socket: IGameSocket, body: Queue, opcode: number) {
  let type = body.byte();
  ...
  try {
    ...
    if (type == TradeType.Buy || type == TradeType.Sell) {
      ...
      if (!npc) throw 0;
      if (npc.npc.type != NPCType.NPC_MERCHANT && npc.npc.type != NPCType.NPC_TINKER) throw 0;
    ...
    if (type == TradeType.Buy) {
      for (let item of itemPocket) {
        ...
        if (item.pos >= 28 || item.pos < 0 || item.count <= 0 || item.count > 9999) throw 2;
    ...
    if (type == TradeType.Buy) {
      if (npcSellingGroup != 249_000) { // buy items with MONEY
        ...
        socket.send([
          opcode, 1, ...int(c.money), ...int(totalPrice), (npcSellingGroup / 1000) | 0
        ])
      } else { // buy items with NP
        //TODO: Handle this later
      }
    } else if (type == TradeType.Sell) {
      if (npcSellingGroup != 249_000) { // sell items for MONEY
