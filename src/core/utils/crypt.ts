...
export class Crypt {
  ...
  private constructor(private enable: boolean = true) {
    if (enable) {
      ...
      this.pubKey = [0, 0, 0, 0, 0, 0, 0, 0];
    }
  }

  public static createInstance() {
    return new Crypt(false);
  }
  ...
  public decode(data: Buffer): void {
    // FIXME: does nothing, fix it later
  }
}

function JvEncryptionFast(len, datain, tKey) {
  /* TODO: Not working please fix */
  const dataout = [];

  let rkey = 2157;
  let lkey = (len * 157) & 0xff;
...
function crc32(s, len) {
  ...
  for (var i = 0; i < len; i++)
    crc32val =
      (crc32_tab[((crc32val >>> 0) ^ s[i]) & 0xff] ^ (crc32val >>> 8)) >>> 0;
